...
def select(id): # working
    ...
    if result is not None:
        ...
        vu_point = Vu_point(result['name'], location, country, result['rating'], result['description'], result['visited'], result['id'])
    return vu_point
...
def update(vu_point): # working
    ...
    values = [vu_point.name, vu_point.rating, vu_point.description, vu_point.visited, vu_point.country.id, vu_point.location.id, vu_point.id]
    run_sql(sql, values)